...
| `styles.css` | Light/dark palettes, glass chrome, type scale, Framework7 variable overrides |
| `theme.css` | Generated by `wal-theme.sh` from the pywal cache: accent and dark background |
| `figures/` | Exercise frames (CC BY-SA 4.0, see `figures/LICENSE.md`) |
...
Screen changes are iOS-style push/pop transitions (new screen slides in, old one parallaxes
back); an edge swipe from the left on the guided screen goes back.

Exercise figures are the SVG frames from [Workout Guide](https://github.com/bryllim/workout-guide)
by Bryl Lim, after [Everkinetic](https://github.com/everkinetic/data), CC BY-SA 4.0 (see
`figures/LICENSE.md`). Three frames per lift are shown as CSS masks tinted with the ink colour
and cross-dissolved with a per-lift tempo (slow eccentric, hold, faster concentric) driven by
Motion. `prefers-reduced-motion` freezes on the first frame and disables the view transitions.

## Dark mode and the accent

Dark mode follows the system setting. The dark palette and the accent are generated from
pywal: run `./wal-theme.sh` after changing wallpaper and commit `theme.css`. Without it the
app falls back to the defaults in `styles.css`.
